...
Users can upload an image, which will be stored as a new row prefixed by the user's id. The newly created row is returned so the image can be referred to later. Both multipart/form-data and application/x-url-encoded uploads are supported.
...
"""""""""""
* (ub64): 
Each parameter provided 



RETURNS
The base64-encoded row key for the new image.
