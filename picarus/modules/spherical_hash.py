...
def _update_pivots(pivots, o, m_div_4):
    ...
    for i in range(pivots.shape[0] - 1):
        for j in range(i + 1, pivots.shape[0]):
            f[i, j, :] = .5 * (o[i, j] - m_div_4) / m_div_4 * (pivots[i] - pivots[j])
            f[j, i, :] = -f[i, j, :]
    for i, pivot in enumerate(pivots):
        pivot += np.mean(f[i, :, :], 0)
...
def train(points, num_pivots, eps_m, eps_s, max_iters):
    ...
    for x in range(max_iters):
        ...
        print((np.mean(o - m_div_4) / (m_div_4 * eps_m), np.std(o) / (eps_s * m_div_4)))
        if np.mean(o - m_div_4) <= eps_m * m_div_4 and np.std(o) <= eps_s * m_div_4:
            break
